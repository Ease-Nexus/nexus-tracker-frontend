...
import { CalendarIcon, FileSpreadsheet, FileText, Search, Filter } from "lucide-react"
...
export function ReportsScreen({ completedTimers }: ReportsScreenProps) {
  ...
  const filteredTimers = completedTimers.filter((timer) => {
    const matchesSearch = timer.badgeNumber.toLowerCase().includes(searchTerm.toLowerCase())
    const matchesDateFrom =
      !dateFrom || isAfter(timer.date, startOfDay(dateFrom)) || timer.date.getTime() === startOfDay(dateFrom).getTime()
    const matchesDateTo =
      !dateTo || isBefore(timer.date, endOfDay(dateTo)) || timer.date.getTime() === endOfDay(dateTo).getTime()
  ...
  const exportToCSV = () => {
    const headers = ["Crachá", "Data", "Hora Início", "Hora Término", "Tempo Total"]
    const csvContent = [
      headers.join(","),
    ...
    link.setAttribute("download", `relatorio-timers-${format(new Date(), "yyyy-MM-dd")}.csv`)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Visualize e exporte o histórico de timers finalizados</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Filtre os dados por crachá, período ou ordene os resultados</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Search by badge */}
            <div className="space-y-2">
              <label className="text-sm font-medium">Buscar por crachá</label>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Select value={sortBy} onValueChange={(value: "date" | "badge" | "duration") => setSortBy(value)}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {sortedTimers.length} registro{sortedTimers.length !== 1 ? "s" : ""} encontrado
        ...
        <CardContent>
          {sortedTimers.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">Ajuste os filtros ou finalize alguns timers para ver os dados aqui</p>
            </div>
          ) : (
                ...
                <TableBody>
                  {sortedTimers.map((timer) => (
                    <TableRow key={timer.id}>
                      <TableCell className="font-medium">#{timer.badgeNumber}</TableCell>
                      <TableCell>{format(timer.date, "dd/MM/yyyy", { locale: ptBR })}</TableCell>
                      <TableCell>{format(timer.startTime, "HH:mm", { locale: ptBR })}</TableCell>
                      <TableCell>{format(timer.endTime, "HH:mm", { locale: ptBR })}</TableCell>
                      <TableCell>{formatDuration(timer.totalElapsed)}</TableCell>
                      <TableCell>
