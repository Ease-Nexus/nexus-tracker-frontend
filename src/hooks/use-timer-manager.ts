...
import type { Timer, TimerBlock } from "@/app/page"
...
export function useTimerManager() {
    ...
    try {
      ...
      if (saved) {
        const parsedTimers = JSON.parse(saved).map((timer: any) => ({
          ...
          history: timer.history.map((block: any) => ({
            ...block,
          ...
          if (timer.status === "running" && timer.remainingTime > 0) {
            ...
            if (newRemainingTime === 0) {
              ...
              if (currentBlock && !currentBlock.end) {
                const elapsed = Math.floor((now.getTime() - currentBlock.start.getTime()) / 1000)
  ...
  const saveCompletedTimer = useCallback((timer: Timer, finalHistory: TimerBlock[]) => {
    const totalElapsed = finalHistory.reduce((sum, block) => sum + block.elapsed, 0)
        ...
        prev.map((timer) => {
          if (timer.id === id) {
            ...
            if (updates.status && updates.status !== timer.status) {
              ...
              } else if (timer.status === "running" && (updates.status === "paused" || updates.status === "stopped")) {
  ...
  const getCompletedTimers = useCallback((): CompletedTimer[] => {
    try {
      ...
      if (saved) {
        return JSON.parse(saved).map((timer: any) => ({
          ...timer,
